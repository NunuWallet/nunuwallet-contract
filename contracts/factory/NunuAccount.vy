...
def _check_daily_limit(_value: uint256):
    ...
    assert self.daily_transfer[self.day] <= daily_limit, "Nunu: transfer limit"
...
def token_bound_accounts(_token: address, _token_id: uint256) -> bool:
    ...
    self.token_id = _token_id
    assert NunuFactory(self.factory).storage_accounts(self, _token, _token_id, default_return_value=True) # dev: fail
