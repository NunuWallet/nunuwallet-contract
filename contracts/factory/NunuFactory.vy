...
@external
def __init__(_nunu_account: address, _refund_address: address, _signature_checker: address, _security: address):
    self.owner = msg.sender
    self.nunu_account = _nunu_account
    self.refund_address = _refund_address
    self.signature_checker = _signature_checker
    self.security = _security

    name: String[64] = "Nunu Factory v1"
    NAME = name
...
@internal
def _validate_input(_owner: address, _guardian: address, _chain_id: uint256):
    assert not empty(address) in [_owner, _guardian]
    assert chain.id == _chain_id, "NUNU: Wrong chain"
...
def _validate_and_refund(
    ...
    assert SignatureChecker(self.signature_checker).is_valid_signature_now(_owner, _hash, _owner_signature), "NUNU: invalid signature"
...
def compute_address(_salt: bytes32, _bytecode_hash: bytes32, _deployer: address) -> address:
    assert _deployer != empty(address), "NUNU: empty address"
...
def create_account(_param: AccountParameters) -> address:
    ...
    assert msg.sender != empty(address), "NUNU: empty sender address"

    self._validate_input(_param.owner, _param.proxy, _param.chain_id)
    new_salt: bytes32 = self._new_salt(_param.signer, _param.token, _param.token_id, _param.proxy, _param.salt)
    new_account: address = create_minimal_proxy_to(self.nunu_account, salt=new_salt)

    assert NunuAccount(new_account).initialize(
        _param.owner,
        _param.signer,
        self,
        _param.proxy,
    ...
    assert SecurityManager(self.security).initialize_default_guardian(new_account, DEFAULT_GUARDIAN, default_return_value=True), "Nunu: initialize default guardian fail"
...
def change_wallet(_new_account: address) -> bool:
    ...
    old_account: address = self.nunu_account
    self.nunu_account = _new_account
...
def storage_accounts(_account: address, _token: address, _token_id: uint256) -> bool:
    assert self.is_account[msg.sender], "Empty account"
    assert _account == msg.sender, "Only account"
