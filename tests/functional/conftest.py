...
@pytest.fixture
def new_account(bob, w3, nunu_account, factory, proxy, security, factory_sign_message):
    ...
    byte_code = vyper_proxy_byte_code(str(nunu_account.address))
    byte_code = w3.keccak(hexstr=byte_code.hex()).hex()

    news = encode(['address', 'address', 'address', 'uint256', 'uint256', 'address', 'uint256'],
            [str(nunu_account.address), owner, str(token), token_id, chain_id, proxy_address, salt]).hex()
