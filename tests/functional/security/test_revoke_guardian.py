...
def test_revoke_guardian(bob, w3, proxy, base, security, new_account, base_sign_message):
    ...
    assert security.get_guardian_addition_period(new_account, new_guardian1) == 0
    assert security.get_guardian_count(new_account) == 2

    # ========= roveke guardian =========

    data = security.revoke_guardian.encode_input(new_account, new_guardian1)
    ...
    gas_price = 10000
    gas_limit = 10000
    deadline = pending_time + 18000000000
    refund_token = zero_address
    refund_addres = bob.address
    ...
    assert security.get_guardian_revokation_period(new_account, new_guardian1) != 0

    chain.pending_timestamp += int(time.time()) + 8640000

    security.config_guardian_revoke(new_account, new_guardian1, sender=bob)

    assert security.get_guardian_revokation_period(new_account, new_guardian1) == 0
    assert not security.is_guardian(new_account, new_guardian1)
    assert security.get_guardian_count(new_account) == 1
